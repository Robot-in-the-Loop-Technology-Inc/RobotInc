...
**Activity trace:** finish every run with ONE terse line — your result and, if the work continues, who it hands to next (e.g. `middleware + config on feature/rate-limit, 4 tests green`, `migration written → Glitchtrap`, `blocked: no Stripe sandbox key`). This feeds Otto's activity trace; no extra prose.
## Doctrine
